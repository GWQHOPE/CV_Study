...
def affine_forward(x, w, b):
  ...
  输入 x 的形状为 (N, d_1, ..., d_k)，包含一个 N 个样本的小批量，
  其中每个样本 x[i] 的形状为 (d_1, ..., d_k)。
  我们将每个输入重新塑形为维度为 D = d_1 * ... * d_k 的向量，然后将其转换为维度为 M 的输出向量。
  输入：
  x：一个包含输入数据的 numpy 数组，形状为 (N, d_1, ..., d_k)
  w：一个权重的 numpy 数组，形状为 (D, M)
  b：一个偏置的 numpy 数组，形状为 (M,)
  返回一个元组：
